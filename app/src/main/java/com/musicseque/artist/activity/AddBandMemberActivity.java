...
import com.musicseque.artist.artist_adapters.SearchBandMemberAdapter;
...
public class AddBandMemberActivity extends BaseActivity implements MyInterface {
    ...
    private void hitAPI() {
        if (Utils.isNetworkConnected(this)) {
           showDialog();
            ...
            RetrofitAPI.callAPI(jsonObject.toString(), Constants.GET_ARTIST_LIST, AddBandMemberActivity.this);
        } else {
...
//                recyclerArtist.setAdapter(searchArtistAdapter);
    ...
    public void afterEditTextInput(Editable editable) {
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                if (Utils.isNetworkConnected(AddBandMemberActivity.this)) {
                    JSONObject jsonObject = new JSONObject();
                    try {
                    ...
                    RetrofitAPI.callAPI(jsonObject.toString(), Constants.SEARCH_ARTIST, AddBandMemberActivity.this);

                } else {
                    Utils.showToast(AddBandMemberActivity.this, no_internet);
                }
            }
        }, 1000);
    }
